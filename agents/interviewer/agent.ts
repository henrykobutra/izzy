  ...
  try {
    ...
    if (interviewerResponse.interviewer_response.next_question) {
      const firstQuestionData = {
        ...
        question_text:
          interviewerResponse.interviewer_response.next_question.question_text,
        question_type:
          interviewerResponse.interviewer_response.next_question.question_type,
        related_skill:
          interviewerResponse.interviewer_response.next_question.related_skill,
        difficulty:
          interviewerResponse.interviewer_response.next_question.difficulty,
        focus_area:
          interviewerResponse.interviewer_response.next_question.focus_area,
        question_order: 1,
        source: "interviewer",
      };
      ...
      if (existingQuestion) {
        ...
        interviewerResponse.interviewer_response.next_question.id =
          existingQuestion.id;
        console.log(`Using existing question ID: ${existingQuestion.id}`);
      } else {
        // Insert new question and get the ID
        ...
        if (insertError) {
          console.error("Error inserting first question:", insertError);
        } else if (newQuestion) {
...
export async function continueInterview(
  ...
  console.log("continueInterview called with questionId:", currentQuestionId);
  try {
    // Start timing for performance tracking
    const startTime = Date.now();
    ...
    // Try to save user's answer
    try {
      // If we don't have a question ID, attempt to find the current question
      let questionIdToUse = currentQuestionId;

      if (!questionIdToUse) {
        console.warn(
          "No question ID provided, attempting to find current question"
        );

        // Try to find the most recent question for this session
        const { data: recentQuestions, error: findError } = await supabase
          .from("interview_questions")
          .select("id")
          .eq("session_id", sessionId)
        ...
        if (findError) {
          console.error("Error finding recent question:", findError);
        } else if (recentQuestions && recentQuestions.length > 0) {
          questionIdToUse = recentQuestions[0].id;
          console.log(`Found recent question ID: ${questionIdToUse}`);
        }
      }

      // If we have a question ID (original or found), save the answer
      if (questionIdToUse) {
        const { data: existingAnswers, error: checkError } = await supabase
          .from("user_answers")
          .select("id")
          .eq("question_id", questionIdToUse);

        if (checkError) {
          console.error("Error checking for existing answers:", checkError);
        } else if (existingAnswers && existingAnswers.length > 0) {
          // Answer already exists for this question, overwrite it
          console.log(
            `Overwriting existing answer for question ${questionIdToUse}`
          );
          ...
          if (updateError) {
            ...
            console.log(
              `Successfully updated answer for question ${questionIdToUse}`
            );
          }
        } else {
          // Insert new answer
          ...
          if (insertError) {
            ...
            console.log(
              `Successfully saved answer for question ${questionIdToUse}`
            );
          }
        }
      } else {
        console.error("Could not find a question ID to save the answer");
      }
    } catch (saveError) {
      console.error("Unexpected error saving answer:", saveError);
    }
    ...
    if (
      interviewerResponse.interviewer_response.reaction_type === "conclusion"
    ) {
    ...
    // If we have a new question, store it
      ...
      const nextQuestionData = {
        ...
        focus_area:
          interviewerResponse.interviewer_response.next_question.focus_area,
        question_order:
          interviewerResponse.interviewer_response.interview_status
    ...
    return {
      ...
      data: {
        ...
        status: interviewerResponse.interviewer_response.interview_status,
        is_complete:
          interviewerResponse.interviewer_response.reaction_type ===
          "conclusion",
      },
    };
  } catch (error) {
